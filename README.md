...
- `book.setId(form.getId())`같이 `id`값을 변경하는 로직이 포함되어 있는 서비스를 호출할 때 뒷단이든 앞단에서 호출한 유저가 `item`을 수정할 권한을 체크하는 로직이 필요하다.
...
1. 수정 버튼을 선택하면 `/item/{itemId}/edit` URL을 GET방식으로 요청
...
2. `items/{itemId}/edit`URL을 POST방식으로 요청하고 `updateItem()`메소드를 실행
3. 이때 컨트롤러에 파라미터로 넘어온 `item`엔티티 인스턴스는 현재 준영속 상태다. 따라서 영속성 컨텍스트의 지원을 받을 수 없고 데이터를 수정해도 변경 감지 기능은 동작하지 않는다.
...
(여기서는 `itemService.saveItem(book)`에서 수정을 시도하는 `Book`객체다. `Book`객체는 이미 DB에 한번 저장되어서 식별자는 존재한다. 이렇게 임의로 만들어낸 엔티티도 기존 식별자를 가지고 있으면 준영속 엔티티로 볼 수 있다.)
...
- 서비스 단에서 set 메소드를 사용하여 값을 변경해 주기보다 도메인 단에서 값을 변경해주는게 객체 지향적이고 메소드를 역추적하기 쉽다.
...
상품 리포지토리에선 `save()`메서드를 유심히 봐야 하는데, 이 메서드 하나로 저장과 수정(병합)을 다 처리한다. 코드를 보면 식별자 값이 없으면 새로운 엔티티로 판단해서 `merge()`로 수정(병합)한다. 결국 여기서의 저장(save)이라는 의미는 신규 데이터를 저장하는 것뿐만 아니라 변경된 데이터의 저장이라는 의미도 포함한다. 이렇게 함으로써 이 메서드를 사용하는 클라이언트는 저장과 수정을 구분하지 않아도 되므로 클라이언트의 로직이 단순해진다.

여기서 사용하는 수정(병합)은 준영속 상태의 엔티티를 수정할 때 사용한다. 영속 상태의 엔티티는 변경감지(Dirty Checking)기능이 동작해서 트랜잭션을 커밋할 때 자동으로 수정되므로 별도의 수정 메서드를 호출할 필요가 없고 그런 메서드도 없다.
...
import jpabook.jpause1.service.ItemService;
import jpabook.jpause1.service.MemberService;
import jpabook.jpause1.service.OrderService;
import lombok.RequiredArgsConstructor;
import org.springframework.stereotype.Controller;
import org.springframework.ui.Model;
import org.springframework.web.bind.annotation.GetMapping;
...
- 주문할 회원과 상품 그리고 수량을 선택해서 Submit 버튼을 선택하면 `/order`URL을 POST 방식으로 호출
- `OrderController`의 `order()`메서드 실행
...
*주문 목록 검색 화면(order/orderList.html)*
      ...
      <form th:object="${orderSearch}" class="form-inline">
        ...
        <div class="form-group mx-sm-1 mb-2">
          <select th:field="*{orderStatus}" class="form-control">
            ...
            <option th:each="status : ${T(jpabook.jpause1.domain.OrderStatus).values()}"
                    th:value="${status}"
